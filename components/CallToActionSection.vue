<template>
  <section id="action-section" class="my-20">
    <div class="container px-6 py-20 mx-auto text-center bg-white">
      <h2 class="mb-6 text-4xl font-bold text-center text-black">
        Built-in Apps
      </h2>
      <h3 class="my-4 text-xl text-black">
        Powerful creativity and productivity content such as show porfolio,
        story photographers, and more with architecture store image on
        Cloundaniry and content.
      </h3>
      <button class="px-8 leading-tight tracking-wider text-blue-600">
        <span class="hover:underline">Learn more</span>
        <svg
          class="inline-block w-3 h-3 fill-current"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 320 512"
        >
          <path
            d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569-9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z"
          />
        </svg>
      </button>
    </div>
  </section>
</template>
